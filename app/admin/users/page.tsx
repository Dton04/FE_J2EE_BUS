...
export default function AdminUsersPage() {
  ...
  const handleToggleLock = async (user: StaffResponse) => {
    try {
      ...
      await adminUserService.updateUserLock(user.id, { is_locked: !user.is_locked });
                ...
                {filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="text-sm font-bold text-gray-900 bg-gray-100 px-2.5 py-1 rounded-full">{user.bookings_count ?? 0}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-2 py-1 rounded text-[11px] font-bold uppercase ${
                        !user.is_locked ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                      }`}>
