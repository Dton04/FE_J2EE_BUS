...
import { Ticket, Calendar, MapPin, CheckCircle2, XCircle, Clock, CreditCard, Star } from 'lucide-react';
...
export default function OrdersContent() {
  ...
  const [reviewingBooking, setReviewingBooking] = useState<MyBookingResponse | null>(null);
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-4">
          {isLoading ? (
            ...
            <div className="bg-white border border-gray-100 p-12 rounded-2xl text-center shadow-sm flex flex-col items-center justify-center min-h-[300px]">
              ...
              <Link href="/" className="text-blue-600 bg-blue-50 hover:bg-blue-100 px-6 py-2.5 rounded-xl font-bold transition-colors">
                Tìm chuyến đi ngay
              </Link>
            </div>
          ) : (
            items.map((booking) => {
              ...
              return (
                <div
                  ...
                  className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-shadow relative"
                >
                  {/* Ribbon */}
                  <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${isCompleted ? 'bg-green-400' : 'bg-blue-500'}`} />

                  {/* Card content – clickable to detail */}
                  <Link href={`/profile/orders/${booking.booking_id}`} className="block p-5 pl-7">
                    {/* Header */}
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-4 border-b border-gray-100">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isCompleted && (
                          <span className="inline-flex items-center gap-1 px-2 py-1 bg-yellow-50 text-yellow-600 border border-yellow-100 rounded-full text-[10px] font-bold">
                  ...
                  {isCompleted && (
                    <div className="px-5 pb-4 pl-7">
                      <button
                        id={`order-review-btn-${booking.booking_id}`}
                        onClick={() => {
                          setReviewingBooking(booking);
                          setReviewModalOpen(true);
                        }}
                        className="w-full flex items-center justify-center gap-2 py-2.5 bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-bold rounded-xl transition-all shadow-sm hover:shadow-md text-sm"
                      >
                        <Star size={15} className="fill-current" />
                        Đánh giá chuyến đi này
                      </button>
                    </div>
                  )}
                </div>
      ...
      {/* Review Modal */}
      {reviewModalOpen && reviewingBooking !== null && (
        <ReviewModal
          ...
          bookingId={reviewingBooking.booking_id}
          bookingCode={reviewingBooking.booking_code}
          routeName={reviewingBooking.route_name}
          departureTime={reviewingBooking.departure_time}
        />
      )}
    </>
  );
